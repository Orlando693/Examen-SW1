    ...
    <Box component="main" sx={{ height: '100dvh', overflowY: 'auto', py: { xs: 3, md: 7 }, bgcolor: '#F3F7F9' }}>
      <Container maxWidth="md">
        <Stack spacing={3}>
          ...
          {projects?.map((project) => <Paper key={project.id} sx={{ p: 2, display: 'flex', gap: 2, alignItems: 'center', justifyContent: 'space-between' }}><Box sx={{ minWidth: 0 }}><Typography fontWeight={700}>{project.name}</Typography><Typography variant="body2" color="text.secondary">{project.description ?? 'No description'}</Typography></Box><Stack direction="row" spacing={1}><Button onClick={() => { window.location.assign(`/editor?projectId=${encodeURIComponent(project.id)}`); }}>Open</Button><Button onClick={() => openDialog('rename', project)}>Rename</Button><Button color="error" onClick={() => openDialog('delete', project)}>Delete</Button></Stack></Paper>)}
        </Stack>
      </Container>
