...
describe('RemoteSelectionOverlay', () => {
  ...
  it('uses the current measured React Flow node bounds without viewport offsets or intercepted input', () => { render(<RemoteSelectionOverlay flow={flow} currentUserId="owner" participants={[participant('editor', ['class-1'])]} />); expect(screen.getByTestId('remote-selection-editor-class-1')).toHaveStyle({ left: '120px', top: '80px', width: '240px', height: '140px', boxSizing: 'border-box', pointerEvents: 'none' }); });
