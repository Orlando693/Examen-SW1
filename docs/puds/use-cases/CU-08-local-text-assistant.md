...
- Correccion post-archive `fix-assistant-model-owned-create-ids` (2026-09-19), aceptada y archivada como `openspec/changes/archive/2026-09-19-fix-assistant-model-owned-create-ids`: la causa raiz era que la salida no confiable podia seleccionar IDs de nuevas entidades UML y colisionar con una identidad existente. Grammar y prompt local ya no ofrecen `classId`, `attributeId` ni `relationId` para `create_class`, `add_attribute` o `create_relation`; el adaptador descarta cualquier ID de create de candidatos de bypass antes de `UmlCommand`, y `UmlCommandBus`/executor asigna la identidad confiable. Las referencias por ID de targets existentes se preservan. Regresiones core cubren candidato completo -> preview sin efectos -> apply -> bus -> modelo para clase, atributo y relacion con IDs en colision. El provider determinista y Playwright cubren los tres Apply contra backend/PostgreSQL, verificando una sola entidad nueva, sin duplicado y con endpoints existentes preservados. Evidencia: assistant-core 14/14, local-llm 27/27, backend assistant 17/17, frontend 183/183, Playwright 8/8, raiz equivalente 462/462, smoke real Qwen, `npm run typecheck`, `npm run lint`, `npm run build`, OpenSpec change strict y main specs strict PASS. El benchmark real no se repitio y no hubo `ECONNRESET`.
